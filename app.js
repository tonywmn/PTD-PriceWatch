...
const logos={
 ...
 'Amazon.de':'https://cdn.simpleicons.org/amazon/FF9900',
...
async function load(){
 ...
 sources=ss||[];render();await loadHistory();
}
function render(){
 const valid=sources.filter(s=>s.availability==='available'&&s.last_price!=null).sort((a,b)=>a.last_price-b.last_price);
 const best=valid[0];
 $('#systemStatus').textContent='Monitoring aktiv';
 $('#heroPrice').textContent=best?euro(best.last_price):'Kein Treffer';
 $('#heroText').textContent=best?`${best.providers.name} · verfügbar`:'Derzeit kein bestätigtes Angebot';
 $('#alertPrice').textContent=euro(product.alert_price);$('#targetPrice').textContent=euro(product.target_price);
 $('#lastUpdated').textContent=fmtDate(newest());$('#providerCount').textContent=`${sources.length} Quellen`;
 $('#providerGrid').innerHTML=sources.slice().sort((a,b)=>a.providers.name.localeCompare(b.providers.name)).map(s=>{
  const a=s.availability||'unknown';const label=a==='available'?'Verfügbar':a==='unavailable'?'Nicht verfügbar':'Unbestätigt';
  const type=s.providers.provider_type==='comparison'?'Vergleichsportal':'Direktanbieter';
  const logo=logos[s.providers.name]||'assets/icons/icon-192.svg';
  return `<a class="provider-card" href="${s.product_url}" target="_blank" rel="noopener"><div class="provider-top"><div class="brand"><img class="brand-icon" src="${logo}" alt=""><div><h3>${s.providers.name}</h3><small>${type}</small></div></div><span class="badge ${a}">${label}</span></div><div class="provider-price">${euro(s.last_price)}</div><div class="provider-meta">${s.status||'Noch nicht geprüft'}<br>Aktualisiert: ${fmtDate(s.last_success_at||s.last_checked_at)}</div></a>`;
 }).join('');fillSettings();
}
...
async function triggerManualScan(){
 ...
 if(!session){$('#settingsDialog').showModal();await authState();toast('Bitte zuerst mit deiner privaten E-Mail anmelden');return}
 const previous=newest();$('#refreshBtn').disabled=true;$('#refreshBtn').textContent='…';toast('Preisprüfung wird gestartet');
 const {error}=await sb.functions.invoke('trigger-price-scan',{body:{}});
 if(error){toast(`Start fehlgeschlagen: ${error.message}`);$('#refreshBtn').disabled=false;$('#refreshBtn').textContent='↻';return}
 toast('Preisprüfung läuft');let attempts=0;
 const timer=setInterval(async()=>{attempts++;await load();const current=newest();if((current&&current!==previous)||attempts>=18){clearInterval(timer);$('#refreshBtn').disabled=false;$('#refreshBtn').textContent='↻';toast(current&&current!==previous?'Neue Ergebnisse geladen':'Scan läuft weiter. Später erneut aktualisieren.')}},10000);
}
...
async function enablePush(){
 ...
 const reg=await navigator.serviceWorker.ready;const sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:toBytes(C.vapidPublicKey)});const j=sub.toJSON();
 const {error}=await sb.from('push_subscriptions').insert({endpoint:j.endpoint,p256dh:j.keys.p256dh,auth:j.keys.auth,active:true});
 toast(error&&error.code!=='23505'?'Push konnte nicht gespeichert werden':'Push-Benachrichtigungen sind aktiviert');
}
$('#refreshBtn').onclick=triggerManualScan;
$('#settingsBtn').onclick=$('#settingsTab').onclick=async()=>{$('#settingsDialog').showModal();await authState()};
$('#notifyBtn').onclick=$('#enablePush').onclick=enablePush;
...
sb.auth.onAuthStateChange(authState);load();
