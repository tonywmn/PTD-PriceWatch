    ...
    payload = json.dumps(
        {
            ...
            "url": os.getenv(
                "APP_URL", "https://tonywmn.github.io/PTD-PriceWatch/"
            ),
...
def diagnostics(provider, page, body):
    title = page.title()
    final_url = page.url
    block = blocked_reason(title, body)
    compact = re.sub(r"\s+", " ", body).strip()[:700]
    print(f"DIAG_PROVIDER={provider}")
    print(f"DIAG_TITLE={title}")
    print(f"DIAG_URL={final_url}")
    print(f"DIAG_BLOCK={block or 'none'}")
    print(f"DIAG_TEXT={compact}")
    return title, final_url, block
...
def scan_source(browser, source, products_by_id):
    ...
    try:
        ...
        html = page.content()
        _, final_url, block = diagnostics(provider, page, body)
            ...
            patch.update(
                {
                    ...
                    "error_message": f"Final URL: {final_url}",
                }
            )
        else:
        ...
        if result:
            ...
            if alert:
                ...
                upsert_alert(
                    {"source_id": source["id"], "alert_active": False}
                )

            print(
                f"RESULT {provider}: status={result.status}; "
                f"price={result.price}; availability={result.availability}; "
...
def main():
    ...
    selected = sources if force else [s for s in sources if source_is_due(s)]

    print(f"SCAN_START count={len(selected)} force={force}")
    if not selected:
        return
